...
- [ ] 1.1 Add a test that runs `Watcher.work` against a repo on
...
- [ ] 1.2 Add a test that pins `main`'s SHA before and after the
  run and asserts they are equal (no merge happened, no commit
  added on `main`).
- [ ] 1.3 Add a test that asserts `see/<change>` exists after the
...
- [ ] 1.10 Update the existing test at line ~980 ("expected no
  `see/` branches") to assert `see/<change>` DOES exist at run
  end and is HEAD.
- [ ] 1.11 Update the existing test at line ~840 (drifted
...
- [ ] 1.12 Confirm the failure-path tests still pass without
...
- [ ] 2.7 Confirm the catch-up `git add -A && git commit
...
- [ ] 3.1 Add a `## REMOVED Requirements` block under the
  ...
  contract.
- [ ] 3.2 Add a `## ADDED Requirements` block: a new
...
- [ ] 3.3 Add a `## MODIFIED Requirements` entry: the
...
- [ ] 4.1 Add a new bullet to `shape_choices` documenting that
...
- [ ] 5.1 Run `go build ./...` and confirm it compiles.
- [ ] 5.2 Run `go test ./...` and confirm all tests pass,
  including the rewritten merge-related tests (tasks 1.5–1.10)
  and the unchanged failure-path tests (task 1.12).
- [ ] 5.3 Run `openspec validate remove-merge-step` and
  confirm green.
