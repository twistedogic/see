...
For each watched repository in custom mode, `see` SHALL execute the configured condition in that repository through the platform shell with the watcher context. Unix-like systems SHALL use `/bin/sh -c`; Windows SHALL use `cmd.exe /C`. Exit status `0` SHALL mean work is available, exit status `1` SHALL mean no work is available, and a shell launch failure or any other nonzero exit status SHALL be a condition error. A condition error SHALL retain captured standard error in its diagnostic when standard error is nonempty.
