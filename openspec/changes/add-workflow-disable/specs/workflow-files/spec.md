...
### Requirement: Frontmatter holds condition, commit, model, disable, and an ignored name
...
- **WHEN** `openspec.md` contains frontmatter with `name`, `condition`, `commit`, `model`, and `disable`
...
- **AND** the workflow's `disable` is the value of the `disable` key (or `false` if absent)
