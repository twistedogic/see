...
- [ ] 1.1 Replace `Watches []string` on `Config` with `RootDir string`,
      `Include []string`, `Exclude []string`. Update `Config` struct,
      keep `Prompt`, `Condition`, `Commit` unchanged.
- [ ] 1.2 Add a private `validateConfig(cfg *Config) error` in
...
- [ ] 1.3 Call `validateConfig` from `loadConfig` after decode,
      before returning. Keep the existing strict decoder, multi-doc
      check, and missing-file / empty-file handling unchanged.
- [ ] 1.4 Keep `validateCustomConfig` as-is; call it from `main` as
      today.

## 2. Resolver

- [ ] 2.1 Add `resolveConfiguredTargets(cfg Config) ([]string,
...
- [ ] 5.1 `config_test.go`: rewrite every `watches:` fixture to
...
- [ ] 5.2 `config_test.go`: add tests for the new
      ...
      path.
- [ ] 5.3 `discovery_test.go`: delete the `resolveWatchList*` tests
...
- [ ] 5.4 `main_test.go`: grep for `--watch` and `watchFlag`,
      ...
      intentionally.
- [ ] 5.5 Run `go test -timeout 30s ./...` and confirm green.

## 6. Documentation

- [ ] 6.1 Update the "Configuration" section of `AGENTS.md` so the
      schema reference lists `root_dir`, `include`, `exclude`
      instead of `watches`. Update the example block.
- [ ] 6.2 Update the migration note in `AGENTS.md` (the legacy
...
- [ ] 7.1 Run `go vet ./...` and `go test -timeout 30s ./...`;
      both must be green.
- [ ] 7.2 Run `rg -n 'watches\b|Watches\b|watchFlag\b|multiFlag\b|
...
- [ ] 7.3 `openspec validate root-dir-include-exclude` (or
