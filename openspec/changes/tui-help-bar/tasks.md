...
- [ ] 1.1 Create `tui/keys.go` with a `keymap` struct that holds
...
- [ ] 2.1 In `tui/model.go`, replace the `msg.String() == "q" ||
      ...
      ```
- [ ] 2.2 In the `tea.WindowSizeMsg` branch, add
      `m.help.SetWidth(msg.Width)` next to the existing
      `m.width = msg.Width` assignment.
- [ ] 2.3 Confirm `TestUpdateQuitsOnQ` in `tui/tui_test.go`
...
- [ ] 3.1 In `tui/view.go`, delete the `tickerQuit` constant and
...
- [ ] 3.2 Rewrite `renderFooter` to return three rows joined by
      `\n`:
      ```go
...
- [ ] 3.3 Add `renderSeparatorLine()`: a styled `─` repeated to
...
- [ ] 3.4 Update `tickerMiddleWidth`: drop the
      ...
      unchanged.
- [ ] 3.5 In `renderContent`, replace the `fixedLines := 3`
      literal with:
      ```go
      helpLines := 1
      if m.help.ShowAll {
      ...
      ```
- [ ] 3.6 Run `go build ./...` and `go test -timeout 30s
...
- [ ] 4.3 In `tui/ticker_test.go`, update
...
- [ ] 5.3 In `tui/tui_test.go`, add `TestHelpWidthTracksWindowSize`:
      ...
      fits.
- [ ] 5.4 Run `go test -timeout 30s ./tui/...`. Expect green.

## 6. Final validation and review

- [ ] 6.1 Run `go vet ./...`, `go build ./...`,
      `go test -race -timeout 30s ./...`. Expect green.
- [ ] 6.2 Run `openspec validate tui-help-bar`. Expect green; the
      change is complete with `proposal.md`, `design.md`,
      `specs/tui/spec.md`, and `tasks.md` populated.
- [ ] 6.3 Manual smoke check (not CI): build the binary, run
...
- [ ] 6.4 Run `openspec archive tui-help-bar --yes` to sync the
