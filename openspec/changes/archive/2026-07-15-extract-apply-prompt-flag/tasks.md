...
- [x] 1.1 Extend `fakeAgent` in `main_test.go` with a `prompts []string`
  recorder that captures the 4th `Run` argument on every call
- [x] 1.2 Add `TestRenderPromptSubstitutesChange` asserting that
...
- [x] 1.3 Add `TestDefaultTemplateMentionsChange` asserting that
  `renderPrompt(defaultPromptTemplate, "add-foo")` contains the
  substring `"add-foo"`
- [x] 1.4 Add `TestWatcherRendersUserPrompt` constructing a
...
- [x] 1.5 Add `TestWatcherFallsBackToEmbeddedDefault` exercising
  `SetPromptTemplate("   ")` and asserting the embedded default
  flows through to `Agent.Run`

## 2. Implementation

- [x] 2.1 Create `prompt.md` at the repo root with the existing
  ...
  ```
- [x] 2.2 Replace `applyPrompt` in `main.go` with
...
- [x] 2.5 Update `Watcher.work` to resolve the effective template
...
- [x] 2.6 Add a `-prompt` flag in `main()` of type `flag.String`
...
- [x] 3.1 Delete the now-unused `applyPrompt` function from
  `main.go`
- [x] 3.2 Delete `TestApplyPromptMentionsChange` from
  `main_test.go` (superseded by `TestDefaultTemplateMentionsChange`
  and `TestRenderPromptSubstitutesChange`)
- [x] 3.3 Verify no other references to `applyPrompt` remain
  (`rg applyPrompt main.go main_test.go` → empty)

## 4. Verify

- [x] 4.1 `go build ./...` succeeds (proves the `//go:embed`
  directive resolves against `prompt.md`)
- [x] 4.2 `go test ./...` passes (covers unit + end-to-end contract)
- [x] 4.3 Spot-check: build and run `see --once` against this repo
