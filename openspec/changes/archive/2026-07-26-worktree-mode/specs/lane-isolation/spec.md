...
If any of steps 2, 3, or 4 fails, `see` SHALL execute the rollback
described in the rollback requirement (worktree removed, lane
...
When worktree mode is active and `auto_merge` is false, on a
successful agent run `see` SHALL:
...
The worktree directory and `see/<digest>` branch SHALL remain in
place after the run for the operator to inspect and merge manually.

On agent failure, rollback SHALL apply as for auto-merge (worktree
removed, lane deleted).
...
- **WHEN** worktree mode with `auto_merge: false` is active and the
  agent succeeded
- **THEN** `see/<digest>` is rebased onto the operator's current
  branch tip
- **AND** the worktree directory and `see/<digest>` remain in place
  for manual review
...
When worktree mode is active, on any failure path (agent error, rebase
error, merge error, or pre-attempt check) `see` SHALL execute the
following cleanup steps, in order, ignoring individual step failures
and emitting `Warning` events for any step that fails:
...
- **WHEN** worktree mode is active and the attempt fails for any
  reason
- **THEN** the worktree directory is removed (or `Warning` is
  emitted if the remove fails)
...
- **WHEN** a previous attempt failed and left a stale worktree
  directory
- **THEN** the next attempt's `git worktree prune` removes the stale
  metadata
- **AND** `git worktree add -B` creates a fresh worktree on the
