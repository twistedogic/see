...
- [x] 1.1 Add `loadWatchConfig() ([]string, error)` in
...
- [x] 1.2 Implement tilde expansion for each non-comment
  line: replace a leading `~` with `$HOME` (or
  `os.UserHomeDir()` when `$HOME` is unset). Do not
  expand `$VAR` patterns.
- [x] 1.3 Add a test that writes a known config to a
  ...
  expectations.
- [x] 1.4 Add a test that asserts a malformed line
  returns an error carrying the line number.
- [x] 1.5 Add a test that asserts a missing config file
  returns `(nil, nil)`.

## 2. Discovery layer (`discovery.go`)

- [x] 2.1 Implement `resolveTargets(patterns []string)
...
- [x] 3.3 Remove the `os` import or the readdir
  call site if no other uses remain.

## 4. main wiring

- [x] 4.1 Add two new flags: `--watch` (a
  `flag.Var`-driven string slice via a small
  `multiFlag` helper type) and `--ignore-config`
  (bool).
- [x] 4.2 Replace the `os.Getwd()` block in `main()`
...
- [x] 4.3 Plumb the resolved repo slice into
  `runTUI(ctx, &w, events, repos)` and `w.Watch(ctx,
  repos)`.
- [x] 4.4 When the resolution layer emits warnings,
...
- [x] 4.5 Update `--help` text via the new flags'
...
- [x] 5.1 Add a test that creates a temp directory
...
- [x] 5.2 Add a test that runs the resolution layer
...
- [x] 5.3 Add a test that writes a config file, calls
  the resolution layer with `--ignore-config = true`
  and a single `--watch`, and asserts the config file
  is not consulted.
- [x] 5.4 Update existing tests that called
  `Watcher.Watch(ctx, wd)` to call the new signature
  with an explicit path list.

## 6. Validation

- [x] 6.1 Run `go build ./...` and `go test ./...` and
  confirm green.
- [x] 6.2 Run `openspec validate add-watch-paths-config`
