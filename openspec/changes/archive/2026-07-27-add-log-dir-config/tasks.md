...
- [x] 1.1 Add `LogDir string \`yaml:"log_dir"\`` to the `Config`
...
- [x] 2.1 In `eventlog.go`, replace `ensureLogDir()` with
      ...
      contract.
- [x] 2.2 Update the `ensureLogDir` doc comment (now on
...
- [x] 2.3 Audit every `ensureLogDir` call site (`rg ensureLogDir`).
      The only production caller is `main.go`; tests that referenced
      `ensureLogDir` directly are updated in section 5.

## 3. main() reorder

- [x] 3.1 In `main.go`, move the `loadStartupConfig(*configFlag)`
      ...
      unaffected.
- [x] 3.2 Replace `logDir, err := ensureLogDir()` with
      `logDir, err := resolveLogDir(cfg)`. Keep the
      `openEventLogger(logDir)` and `NewWatcher(*pi, logDir, ...)`
      calls exactly as-is.
- [x] 3.3 Confirm no step between the moved `loadStartupConfig` and
...
- [x] 4.1 Add a commented `# log_dir: "~/.cache/see/logs"` line to
...
- [x] 5.1 `config_test.go`: add `validateConfig` cases for `log_dir`:
      ...
      `log_dir` field path.
- [x] 5.2 `eventlog_test.go` (or a new test in the package): add
      ...
      is a file).
- [x] 5.3 Update any existing test that called `ensureLogDir`
      directly to call `resolveLogDir(Config{})` (or with an explicit
      `LogDir`), preserving the directory-creation assertion.
- [x] 5.4 Run `go test -timeout 30s ./...` and confirm green.

## 6. Documentation

- [x] 6.1 Update the "Configuration" section of `AGENTS.md`: add
...
- [x] 7.1 Run `go vet ./...` and `go test -timeout 30s ./...`; both
      must be green.
- [x] 7.2 Run `rg -n 'ensureLogDir|os\.UserCacheDir' --include='*.go'`
      and confirm no production references to the removed function or
      the old default remain (only `resolveLogDir` and
      `defaultLogDir`).
- [x] 7.3 `openspec validate add-log-dir-config` reports the change
