...
- [ ] 1.1 In `tui/tui_test.go`, add a regression test that drives one
...
- [ ] 2.1 In `tui/view.go` `renderRow`, delete the
      `if r.LogPath != "" { row += "\n      " + r.LogPath }` block so
      every row is a single `lipgloss.JoinHorizontal` line.
- [ ] 2.2 In `tui/view.go` `fitToHeight`, collapse to single-line
      accounting: drop the `rowLines = 2` branch and the
      log-path-continuation comment; every retained row costs exactly
      one line.
- [ ] 2.3 Confirm the 1.1 regression test now passes.

## 3. Delete the dead TUI-side plumbing

- [ ] 3.1 In `tui/model.go`, remove the `case LogPathMsg` arm of
      `Update` and the `LogPath string` field on `RepoRow`.
- [ ] 3.2 In `tui/events.go`, remove the `LogPathMsg` type.
- [ ] 3.3 In `main.go`, remove the `case LogPath` arm of
...
- [ ] 4.1 Delete `tui/tui_test.go` `TestViewRendersLogPathWhenSet`
      (it asserts the now-removed render).
- [ ] 4.2 Rework the 1.1 regression test so it no longer depends on
      ...
      each).
- [ ] 4.3 Confirm `TestViewOmitsLogPathWhenUnset` still holds as the
      baseline one-line row shape (summary + header + 1 row + footer).

## 5. Verification

- [ ] 5.1 Run `go vet ./...` and `go test -timeout 30s ./...`; both
      must be green.
- [ ] 5.2 Run `rg -n 'LogPathMsg|r\.LogPath|case LogPath' tui/ main.go`
      and confirm no TUI-side references remain — the only hits are
      the `main.LogPath` event type and its emit sites in `main.go`.
- [ ] 5.3 `openspec validate remove-tui-log-path` reports the change
