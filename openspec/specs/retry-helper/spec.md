...
Define the retry contract used by `see` to re-invoke work on a repository. The
contract guarantees that `retryN` reports the *last* error when every attempt
fails, so the watcher cannot claim silent success.
